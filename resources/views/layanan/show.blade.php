    ...
    <div class="layanan-detail-grid">
              ...
              <ol style="list-style:none; display:flex; flex-direction:column; gap:0;" aria-label="Tahapan proses layanan">
                @foreach($layanan->proses as $i => $step)
                  <li style="display:flex; align-items:flex-start; gap:var(--space-4); padding:var(--space-3) 0; border-bottom:1px dashed var(--clr-border);">
                    <div style="width:24px; height:24px; background:var(--clr-text); color:white; border-radius:50%; display:flex; align-items:center; justify-content:center; font-family:var(--font-heading); font-weight:700; font-size:0.75rem; flex-shrink:0; margin-top:1px;" aria-hidden="true">{{ $i+1 }}</div>
                    <div style="flex:1;">
      ...
      <aside class="layanan-detail-sidebar" aria-label="Navigasi layanan">
            ...
            @foreach($semuaLayanan as $item)
              <li role="listitem">
                <a href="{{ route('layanan.show', $item->slug) }}"
                   class="layanan-sidebar-link {{ $item->id === $layanan->id ? 'active' : '' }}"
        ...
        <div style="background:var(--clr-text); border-radius:var(--radius-lg); padding:var(--space-6);">
          ...
          <a href="https://wa.me/{{ preg_replace('/^0/', '62', preg_replace('/[^0-9]/', '', \App\Models\Setting::get('kontak_telpon', '082233392179'))) }}?text={{ urlencode('Halo, saya tertarik dengan layanan ' . $layanan->nama . '. Boleh konsultasi lebih lanjut?') }}" class="btn btn--white btn--full" target="_blank" rel="noopener noreferrer">
