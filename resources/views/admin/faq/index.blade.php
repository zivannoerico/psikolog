<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4">
            ...
            <a href="{{ route('admin.faq.create') }}" class="inline-flex items-center justify-center px-5 py-2.5 bg-indigo-600 hover:bg-indigo-700 text-white font-semibold rounded-xl transition-all shadow-sm shadow-indigo-200 text-sm gap-2">
    ...
    <div class="bg-white rounded-3xl shadow-sm border border-slate-100 overflow-hidden">
        ...
        <div class="p-6 sm:p-8 border-b border-slate-100 bg-slate-50/50">
            <form action="{{ route('admin.faq.index') }}" method="GET" class="w-full sm:max-w-md relative">
                <input type="text" name="search" value="{{ request('search') }}" placeholder="Cari pertanyaan atau jawaban..." class="w-full pl-11 pr-4 py-3 rounded-xl border border-slate-200 focus:outline-none focus:ring-2 focus:ring-indigo-500/20 focus:border-indigo-500 transition-all text-sm shadow-sm">
                    ...
                    @forelse($faqs as $faq)
                        <tr class="hover:bg-slate-50/50 transition-colors group">
                            <td class="px-6 sm:px-8 py-4">
                                <div class="font-bold text-slate-800 line-clamp-2 max-w-lg" title="{{ $faq->pertanyaan }}">{{ $faq->pertanyaan }}</div>
                            </td>
                            <td class="px-6 sm:px-8 py-4 whitespace-nowrap">
                                <div class="text-sm font-medium text-slate-600 bg-slate-100 px-3 py-1 rounded-lg inline-block border border-slate-200">{{ $faq->urutan }}</div>
                            </td>
                            <td class="px-6 sm:px-8 py-4 whitespace-nowrap">
                                @if($faq->aktif)
                            ...
                            <td class="px-6 sm:px-8 py-4 whitespace-nowrap text-right text-sm font-medium">
                                <div class="flex justify-end space-x-2">
                                    <a href="{{ route('admin.faq.edit', $faq) }}" class="p-2 bg-white text-indigo-600 rounded-lg hover:bg-indigo-50 border border-slate-200 shadow-sm transition-colors" title="Edit">
                                        <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg>
                                    </a>
                                    <form action="{{ route('admin.faq.destroy', $faq) }}" method="POST" onsubmit="return confirm('Anda yakin ingin menghapus FAQ ini?');" class="inline">
                                        @csrf
                                        @method('DELETE')
                        ...
                        <tr>
                            <td colspan="4" class="px-6 py-12 text-center">
                                <div class="flex flex-col items-center justify-center">
                                    ...
                                    <a href="{{ route('admin.faq.create') }}" class="inline-flex items-center text-sm font-semibold text-indigo-600 hover:text-indigo-700">
