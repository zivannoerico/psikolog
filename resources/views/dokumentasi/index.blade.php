...
@section('title', 'Dokumentasi — An Moerty Psikologi Banyuwangi')
@section('description', 'Ringkasan galeri foto dan video kegiatan, pelatihan, dan layanan An Moerty Psikologi Banyuwangi.')
@section('og_title', 'Dokumentasi — An Moerty Psikologi Banyuwangi')
  ...
  <div class="container">
    ...
    <div class="dok-gallery reveal">
      ...
      <button class="dok-gallery-item" 
              ...
              aria-label="Buka foto: {{ $foto->alt ?? $foto->judul }}"
              type="button">
        <img src="{{ asset('storage/' . $foto->gambar) }}" 
             alt="{{ $foto->alt ?? $foto->judul ?? 'Dokumentasi foto' }}" 
             loading="lazy" 
             class="dok-gallery-img">
        @if($foto->judul)
        <div class="dok-gallery-overlay">
          <span class="dok-gallery-caption">{{ $foto->judul }}</span>
        </div>
        @endif
      </button>
      @endforeach
    ...
    <div class="section-footer reveal" style="margin-top: 2rem;">
      <a href="{{ route('dokumentasi.foto') }}" class="btn btn--outline">
        <span>Lihat Semua Foto Dokumentasi ({{ \App\Models\Galeri::aktif()->count() }})</span>
...
<section class="section section--alt" id="dok-video" aria-labelledby="dok-video-heading">
  <div class="container">
    ...
    <div class="dok-video-grid reveal">
      ...
      <div class="dok-video-card">
        <button class="dok-video-thumb" 
                onclick="openVideoModal('{{ $video->embed_url }}', '{{ e($video->judul) }}')"
        ...
        <div class="dok-video-info">
          <h3 class="dok-video-title">{{ $video->judul }}</h3>
          @if($video->deskripsi)
          <p class="dok-video-desc">{{ Str::limit($video->deskripsi, 90) }}</p>
          @endif
        </div>
      </div>
      @endforeach
    ...
    <div class="section-footer reveal" style="margin-top: 2rem;">
      <a href="{{ route('dokumentasi.video') }}" class="btn btn--outline">
        <span>Lihat Semua Video Dokumentasi ({{ \App\Models\DokumentasiVideo::aktif()->count() }})</span>
        <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14"></path><path d="M12 5l7 7-7 7"></path></svg>
      </a>
    </div>
    @else
...
<script>
const lightboxData = @json($fotos->map(fn($f) => ['src' => asset('storage/' . $f->gambar), 'caption' => $f->judul ?? '', 'alt' => $f->alt ?? $f->judul ?? 'Dokumentasi foto']));
